...
- **TeamMember:** `id`, `teamId`, `presencaId` (único — um jogador só pode estar em um time por baba).
- **GameEvent:** `id`, `babaDoDiaId`, `presencaId`, `teamId?`, `type` (GOL/CARTAO_*), `registeredBy`, `createdAt`. Índice: `babaDoDiaId`.
- **Suspension:** `id`, `originEventId` (único, FK GameEvent), `socioId`/`convidadoId`, `effectiveDate` (data do cartão + 7 dias), `liftedAt`/`liftedBy`/`liftReason` (override manual, RF30). Índice: `effectiveDate`.
